...
const DesktopProfile = () => {
  ...
  const on_load_fun = async () => {
    ...
    if (check_login) {
      const user_data = await get_user_data_from_uid(sessionStorage.getItem("uid"));
  ...
  const update_information = async () => {
    ...
    // console.log(res)
    const res = await update_user_data(sessionStorage.getItem("uid"), updated_data);
    if (res) {
      on_load_fun();
    } else {
      set_show_error("block");
  ...
  return (
    <div className="desktop-profile">
      ...
      <NavBar active={4} show_notification_icon={"none"} />
      <NotificationBar />
    </div>
  );
};
